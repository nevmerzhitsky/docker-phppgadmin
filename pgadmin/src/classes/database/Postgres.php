...
class Postgres extends ADODB_base {
    ...
    var $selectOps = array(
        '=' => 'i',  'LIKE' => 'i',     'IS NULL' => 'p',
        '!=' => 'i', 'NOT LIKE' => 'i', 'IS NOT NULL' => 'p',
        'IN' => 'x', 'NOT IN' => 'x',
        '@>' => 'i',
        '<' => 'i', '>' => 'i', '<=' => 'i', '>=' => 'i',
        '<<' => 'i', '>>' => 'i', '<<=' => 'i', '>>=' => 'i',
        'ILIKE' => 'i', 'NOT ILIKE' => 'i', 'SIMILAR TO' => 'i',
    ...
    function printField($name, $value, $type, $extras = array()) {
        ...
        switch (substr($type,0,9)) {
            ...
            case 'bytea':
                ...
                echo "<input type=\"file\" id=\"{$uniqid}_up\" name=\"file_" . htmlspecialchars($name) . "\"{$extra_str} style=\"display:none\"/>";
                break;
    ...
    function getCaptionSql($namespace, $table)
    {
        ...
        while (!$rs->EOF) {
            if (trim($rs->fields['fields'])) {
                $coalesceParts[] = join(" || ',' || ", explode(" ", $rs->fields['fields']));
            }
            $rs->moveNext();
        }
        if (!count($coalesceParts)) {
            // Nothing is found. Just print table name.
            return "'" . $this->clean($table) . "'";
        } else if (count($coalesceParts) == 1) {
            return reset($coalesceParts);
        } else {
            return "COALESCE(" . join(", ", $coalesceParts) . ")";
        }
    }
