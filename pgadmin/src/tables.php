    ...
    function doSelectRows($confirm, $msg = '') {
        ...
        if ($confirm) {
            ...
            if ($attrs->recordCount() > 0) {
                ...
                while (!$attrs->EOF) {
                    ...
                    foreach (array_keys($data->selectOps) as $v) {
                        if (isset($_REQUEST['ops'][$attrs->fields['attname']])) {
                            ...
                            if (preg_match('/\[\]/i', $attrs->fields['type'])) {
                                $vReq = '@>';
                            } else if (preg_match('/\b(character|text|bytea)\b/i', $attrs->fields['type'])) {
                                $vReq = 'LIKE';
                            } else {
                                $vReq = null;
                            }
                ...
                // Select all checkbox
                echo "<tr><td colspan=\"5\"><input type=\"checkbox\" id=\"selectall\" " . (!$_POST? "checked=\"checked\" " : "") . "name=\"selectall\" onclick=\"javascript:selectAll()\" /><label for=\"selectall\">{$lang['strselectallfields']}</label></td>";
                echo "</tr></table>\n";
            }
